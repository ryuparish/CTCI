...
// Non circular for the ease in the union program void print_list(link*& start){ // You do not need to allocate a link object for the walker. That will only lead to lost data.
void print_kth_element(link*& start, int& k){
    link* walker = NULL;
    link* walker_brother = NULL;
    int counter = 1;
    walker = start->next;
    while(walker != NULL){
        // Start walker_brother's k delayed walk
        if(counter == k){
            walker_brother = start->next;
        }
        if(counter > k){
            walker_brother = walker_brother->next;
        }
        walker = walker->next;
        counter++;
    }
    if(walker_brother != NULL){
        std::cout << walker_brother->value << " is the kth element from the last\n";
    }
    else{
        std::cout << "Not enough elements to have a kth to last spot\n";
    }
}
...
int main() {
    ...
    std::cout << "\n";
    print_kth_element(myList->headNode, k);
    delete(myList);
    return 0;
}
